...
void TTKSpeedMeterWidget::drawMark(QPainter *painter)
{
    ...
    qreal dAngle=(qreal)270/100;  // 270
...
void TTKSpeedMeterWidget::drawTextRect(QPainter *painter)
{
    ...
    QPointF topLeftPot(m_center.x() -2.5*rectWidth, m_center.y() - rectWidth*4.1);
    ...
    QPointF valTopLeftPot(m_center.x() -2.5*rectWidth, m_center.y() + rectWidth*2);
    ...
    font.setPointSize(fontSize);
    painter->setFont(font);
    painter->drawText(valTextRect, Qt::AlignCenter, QString("%1").arg(m_value * 10.0));
    painter->restore();
}
