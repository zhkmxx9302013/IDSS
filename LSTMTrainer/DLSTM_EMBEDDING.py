...
class DLSTM():
    ...
    def get_pre_scalar(self):
        sequence_length = 120
        pred_length = 60
        FEATURE_NUM = 7
        dataset = pd.read_csv('processed_data/out3.csv', encoding='gb2312', header=0, index_col=0)
        dataset = dataset.append(pd.read_csv('processed_data/out1.csv', encoding='gb2312', header=0, index_col=0))
        ...
        y_cols = [sequence_length * FEATURE_NUM + item for item in range(pred_length * FEATURE_NUM) if
                  (item + 1) % FEATURE_NUM == 0]  # 选取预测的转速列
    ...
    def load_train_data(self, dataset_path='', sequence_length=60, pred_length=60, test_rto=0.1):
        ...
        tmp = self.X_train.shape[0] / sequence_length
    ...
    def do_predict(self, path=''):
        self.load_data(path, sequence_length=120, pred_length=30)
        x = self.X_train
        y = self.y_train
        # batch_size = 512
        model = load_model('model/double_data13_100_200_model.h5')
        model.load_weights('model/double_data13__100_200_weight.h5')
    ...
    def run_network(self, datapath='', pretrained=False):
        ...
        :return:
        """
        model_path = 'model/double_data13_100_200_model.h5'
        weight_path = 'model/double_data13__100_200_weight.h5'#'./model/' + str(time.strftime('%Y%m%d:%H_%M',time.localtime(time.time()))) + '_DLSTM.h5'
        ...
        self.load_train_data('processed_data/out3.csv', sequence_length, 60, ratio)
        self.load_test_data('processed_data/out2.csv', sequence_length, 60, ratio)

        print(self.scaler.mean_, self.scaler.var_)
        print('\nData Loaded. Compiling...\n')

        tb_cb = keras.callbacks.TensorBoard(log_dir='log/double_data13_100_200_unstateful', write_images=0, histogram_freq=5)
        cbks = [tb_cb]
